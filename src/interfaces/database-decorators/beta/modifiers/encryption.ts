...
function encodeDate(key: string, value: any): any {
  return value instanceof Date ? { value: value.toUTCString(), type: 'Date' } : value;
}

function decodeDate(key: string, value: any): any {
  if (value !== null && typeof value === 'object' && value.type === 'Date' && typeof value.value === 'string') {
...
export class EncryptionModifier extends TwoWayModifier<LockedType, [], Meta, Options> {
  ...
  public override lock(_locked_value: LockedType | undefined, value: unknown): LockedType {
    ...
    const encrypted = Buffer.concat([cipher.update(JSON.stringify(value, encodeDate)), cipher.final()]).toString(
      'base64',
    );
